...
class image {
    ...
    static function thumb($image, $thumbname, $type='', $maxWidth=200, $maxHeight=50, $interlace=true) {
        ...
        if ($info !== false) {
            ...
            //2012-11-07 修复PNG无法生成缩略图
            if ( $type == 'jpg' ||  $type == 'jpeg' ){
            	$imageFun($thumbImg, $thumbname, 99);
            }else{
     ...
     * @return string
     */
    static function buildString($string='SimpleCMS', $rgb=array(), $filename='', $type='png', $disturb=1, $border=true) {
        if (is_string($size))
            $size = explode(',', $size);
        $width = $size[0];
        $height = $size[1];
    ...
    static function buildImageVerify($length=4, $mode=1, $type='png', $width=48, $height=22, $verifyName='verify') {
        include_once 'system/lib/string.class.php';
